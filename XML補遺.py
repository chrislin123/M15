...
while start_dt <= end_dt:
    ...
    server = r"BV550234820001\SQLEXPRESS"
    database = "Procal"
    username = "npust"
    ...
    print(
        "連接資料庫:" + "DRIVER={SQL Server};SERVER=" + server + ";DATABASE=" + database
    )
    ...
    while row:
        ...
        match str(row[2]):
            ...
            case "伸縮計":
                ...
                QueryDateTime = datetime.strftime(
                    start_dt - timedelta(days=1), "%Y-%m-%d %H:%M:00"
                )
                # print(QueryDateTime)
                # 產生查詢時間
                cursorOLD.execute(
